...
const Input = z.object({
  origin: LatLng,
  destination: LatLng,
  mode: z.enum(["WALKING", "DRIVING"]).optional().default("WALKING"),
});

export const computeSafeRoutes = createServerFn({ method: "POST" })
  .inputValidator((data: unknown) => Input.parse(data))
  .handler(async ({ data }) =>
    computeRoutes(data.origin, data.destination, { mode: data.mode })
  );
...
export const computeFastestRoute = createServerFn({ method: "POST" })
  .inputValidator((data: unknown) => Input.parse(data))
  .handler(async ({ data }) =>
    computeFastest(data.origin, data.destination, { mode: data.mode })
  );
